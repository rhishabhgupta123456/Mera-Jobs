...
import tech.merajobs.databinding.ItemPresentationBinding
import tech.merajobs.databinding.ItemSkillBinding
import tech.merajobs.databinding.ItemWorkSampleBinding


class PresentationAdapter(private var dataList: ArrayList<PresentationList>, private var  requireActivity : Activity) :
    RecyclerView.Adapter<PresentationAdapter.ViewHolder>() {

    interface OnRequestAction {
        fun editItem(item: PresentationList)
        fun deleteItem(item: PresentationList)
    }
    ...
    class ViewHolder(val binding: ItemPresentationBinding, private var requireActivity: Activity) :
        RecyclerView.ViewHolder(binding.root) {

        @SuppressLint("SetTextI18n")
        fun bind(dataItem: PresentationList, requireActivity: Activity) {
    ...
    private fun openMenu(item: ImageView, dataItem: PresentationList) {
