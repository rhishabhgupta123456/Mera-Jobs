...
import tech.merajobs.databinding.ItemWhitePaperBinding
import tech.merajobs.databinding.ItemWorkSampleBinding


class WhitePaperAdapter(private var dataList: ArrayList<WhitePaperList>, private var  requireActivity : Activity) :
    RecyclerView.Adapter<WhitePaperAdapter.ViewHolder>() {

    interface OnRequestAction {
        fun editItem(item: WhitePaperList)
        fun deleteItem(item: WhitePaperList)
    }
    ...
    class ViewHolder(val binding: ItemWhitePaperBinding, private var requireActivity: Activity) :
        RecyclerView.ViewHolder(binding.root) {

        @SuppressLint("SetTextI18n")
        fun bind(dataItem: WhitePaperList, requireActivity: Activity) {

            binding.tvTitle.text = dataItem.title
            binding.tvUrl.text =  dataItem.url
            binding.tvDescription.text =  dataItem.description
            binding.tvDate.text = "Published Date : "+ dataItem.published_date
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding =
            ItemWhitePaperBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    private fun openMenu(item: ImageView, dataItem: WhitePaperList) {
